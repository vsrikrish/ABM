...
bf.melt <- melt(bf[[1]], id=c('years', 'rows', 'cols', 'agents'))
...
p <- ggplot() +
  geom_point(data=bf.melt[bf.melt$agents != '25',],
    aes(x=years, y=value, color=agents, shape=agents), size=6) +
  geom_hline(data=bf.levels, aes(yintercept=val), linetype='dashed', size=2) +
  geom_text(data=bf.levels, aes(y=(val+0.35)*1.2, x=40, label=lab), size=6) +
...
+
  facet_wrap(vars(lt), nrow=1, labeller=labeller(lt=lt_labels), strip='left')

#pdf(paste0('figures/bf.pdf'), height=4, width=3)
#p
#dev.off()

png(paste0('figures/poster/bf.png'), height=6.5, width=10, units='in', res=600)
p
dev.off()
