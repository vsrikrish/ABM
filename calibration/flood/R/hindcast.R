...
pcases <- expand.grid(yr=n_yrs, cols=n_cols, rows=n_rows, model=models, lt=lik_types)
...
for (i in 1:nrow(pcases)) {
  ...
  if (rows <= cols) {
    ...
    hind_file <- file.path(hind_path,
      paste('hind-', yrs, '-', rows, '-', cols, '-', mod, '-', seed, '.rds', sep=''))

    hc <- readRDS(hind_file)

    # compute quantiles and medians
    hind[[mod]][[yr]][[agents]][[lik_type]] <-
      lapply(hc, function(x) as.data.frame(t(apply(x, 1, quantile, probs=c(0.025, 0.5, 0.975)))))

    hind[[mod]][[yr]][[agents]][[lik_type]] <-
      lapply(hind[[mod]][[yr]][[agents]][[lik_type]], setNames, c('lower', 'median', 'upper'))

    hind[[mod]][[yr]][[agents]][[lik_type]] <-
      lapply(hind[[mod]][[yr]][[agents]][[lik_type]], function(x) cbind(x, year=1:yrs))
    ...
    obs[[mod]][[yr]][[agents]][[lik_type]] <- data.frame(obs=f$vac, year=1:yrs)
  }
}
...
colnames(hind.melt) <- c('year', 'lower', 'median', 'upper', 'dist', 'lik_type', 'parcels', 'years', 'model')
colnames(obs.melt) <- c('year', 'obs', 'lik_type', 'parcels', 'years', 'model')
...
for (i in 1:nrow(gcases)) {
  ...
  hind_sub <- hind.melt[(hind.melt$model == mod) & (hind.melt$lik_type == lt) & (hind.melt$years != '10') & hind.melt$parcels != '100', ]
  obs_sub <- obs.melt[(obs.melt$model == mod) & (obs.melt$lik_type == lt) & (obs.melt$years != '10') & obs.melt$parcels != '100', ]
  p[[i]] <- ggplot() +
      ...
      geom_point(data=obs_sub, aes(x=year, y=obs, color='obs'), size=0.75, alpha=0.6) +
...
pdf(paste0('figures/hindcast-', seed, '.pdf'),
  width=8, height=8)
print(q)
dev.off()
