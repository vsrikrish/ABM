...
class Resident(Agent):
    ...
    def move(self):
        ...
        nghd = self.model.grid.nghd_list(self.location, r=1, center=False)
        nghd_vac = sum([self.model.grid.is_empty(pos) for pos in nghd])
        x = self.move_params['int'] + \
            self.move_params['self_coef'] * n_flood / mem_years + \
            self.move_params['nghd_coef'] * nghd_vac / len(nghd)
        u = np.random.random()
